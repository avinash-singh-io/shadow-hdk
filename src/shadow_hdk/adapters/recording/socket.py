...
def relay() -> int:
    ...
    assert isinstance(stdin, io.BufferedReader), "stdin is a buffered reader at runtime"
    upward = threading.Thread(target=pump, args=(stdin.read1, upstream.sendall), daemon=True)
    upward.start()
