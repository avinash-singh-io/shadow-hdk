...
class _Turnwise:
    ...
    def recall(self, call: ToolCall) -> None:
        ...
        if (whole := self.held.get(handle)) is None:
            ...
            return
        start = max(0, _an_int(arguments.get("start"), 0))
        length = max(1, _an_int(arguments.get("length"), 4_000))
...
def _an_int(value: JsonValue | None, default: int) -> int:
    """A number the model wrote, or the default — a page's bounds, never a crash on a string."""
