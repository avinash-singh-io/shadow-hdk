...
class CallableComponents(ComponentPort):
    ...
    def remove(self, registration: RegistrationId) -> None:
        """The mirror of `add`: gone from the catalogue on the next refresh, and an invoke by the
        old id is `Failed` — what a battery over a callable does when it is closed, so a closed
        battery offers nothing whatever its kind. `KeyError` for an id never added."""
        del self._entries[registration]
