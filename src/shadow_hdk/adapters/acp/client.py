...
class BridgeClient(acp.Client):
    ...
    async def terminal_output(
        ...
        return schema.TerminalOutputResponse(
            ...
            truncated=truncated,
            exit_status=(
                schema.TerminalExitStatus(exit_code=status.exit_code, signal=status.signal)
                if status
                else None
            ),
        )
    ...
    async def wait_for_terminal_exit(
        ...
        return schema.WaitForTerminalExitResponse(exit_code=ended.exit_code, signal=ended.signal)
