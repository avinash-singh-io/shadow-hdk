...
async def open_battery(
    ...
    if battery.kind == "python":
        ...
        async def forget() -> None:
            # Closed is closed for every kind: a stopped MCP server lists nothing, and so does a
            # callable battery, or a host still holding the port after `aclose` would see a tool
            # that is no longer wanted.
            port.remove(name)

        return OpenedBattery(battery, port=port, _close=forget)
