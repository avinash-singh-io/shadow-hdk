...
A backend proves itself by **denying** something a contained program must not manage (D36). The
sandbox runs that capability test through the backend before it registers anything, so
`contained: true` in the catalogue is an observation rather than a claim. And when the proof
fails, the sandbox does not fall back to a leash and a warning — it refuses to be constructed,
because the person who asked for containment has to be told.
"""
...
def test_a_backend_that_fails_its_proof_is_refused_naming_why(tmp_path: Path) -> None:
    """A box that does not deny the capability is not a box, and the refusal says which capability
    it failed to deny rather than leaving an operator to guess (D36)."""
    with pytest.raises(NotContained) as refused:
        ContainedSandbox(tmp_path, backend=FakeIsolation(contains=False))
    assert "reach a listening socket" in str(refused.value)
...
async def test_the_proof_is_taken_once_not_per_call(tmp_path: Path) -> None:
    """A capability test per step would open a listener and launch a sandbox to re-learn a fact
    about the machine."""
    backend = FakeIsolation(contains=True)
    sandbox = ContainedSandbox(tmp_path, backend=backend)
    probes = [argv for argv in backend.wrapped if "connect_ex" in " ".join(argv)]
    await sandbox.invoke("run_shell", {"command": "echo one"})
    await sandbox.invoke("run_shell", {"command": "echo two"})
    still = [argv for argv in backend.wrapped if "connect_ex" in " ".join(argv)]
    assert len(probes) == 1 and len(still) == 1


def test_a_proof_says_what_was_denied(tmp_path: Path) -> None:
    """A proof nobody can re-read is a flag with a longer name."""
    sandbox = ContainedSandbox(tmp_path, backend=FakeIsolation(contains=True))
    assert "denying" in str(sandbox.proof)
    assert "reach a listening socket" in str(sandbox.proof)
