...
| 9 — the wire | `phase-9-the-wire` | **in progress** | 3 / 5 groups. D19 state-as-JSON and D20 `Spent`; the ports invert over a loopback with D21's crossed context; `--stdio` drives a real child process. Plus **MIT** (O3), **D22** the open port set (O4), and `specs/architecture/refusal.md` answering **J1**. 427 tests; mypy strict over 85 files. Left: `serve` which listens, then schemas and v0.1.0. |
