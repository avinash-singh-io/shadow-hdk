...
| 9 — the wire | `phase-9-the-wire` | **in progress** | 1 / 5 groups. What crosses is settled: the graph's state holds JSON (D19, closing TD-001) and `Spent` is the eleventh event kind (D20, paying Phase 1's debt). Every package at **0.6.0**. 406 tests; mypy strict over 81 files. Left: the protocol and the loopback, `--stdio`, `serve`, then the schemas and v0.1.0. |
