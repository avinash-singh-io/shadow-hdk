...
| 11 — contained sandboxes | `phase-11-contained-sandboxes` | **complete here, unmerged** | 3 / 3 groups. A sandbox proves containment at construction or refuses to exist (D25); gVisor and Firecracker as backends whose live proofs **skip** on this machine and are `[~]` for a Linux host. The leash moved into the runtime so no adapter imports another. 517 tests; mypy strict over 93 files. |
