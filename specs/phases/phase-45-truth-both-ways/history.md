...
### [NOTE] 2026-09-20 — G5: the live measurements, on this machine's signed-in CLIs
Topics: live, codex, claude-code, cache-tokens, behaviour, session-gone, compose
Affects-phases: phase-45-truth-both-ways, phase-36-plan-admission
Affects-specs: none
Detail: Claude Code 2.1.278, Codex 0.154.0, OpenCode 1.18.21 all `ready` here. **Phase 36's owed half:** `uv run pytest -m live tests/test_a_cli_plans_through_the_socket.py` — Claude Code, the library's first ready provider — **passed** in 8.6 s (the model used `compose`; an admission on the record), so the relay-through-the-socket proof now stands on both CLIs. **ENH-028:** a Codex session opened with `Behaviour(model="gpt-5.6-sol", effort="low")` launched `codex exec --json --skip-git-repo-check --sandbox read-only -m gpt-5.6-sol -c model_reasoning_effort="low"`, `unmapped == ()`, and answered `ok` with `reasoning_output_tokens: 0`. **ENH-023:** the same Codex turn: `Usage(input_tokens=17393, output_tokens=5, cache_read_tokens=12032, cache_write_tokens=0)`; a Claude Code turn: `Usage(input_tokens=2, output_tokens=4, cost_cents=3, cache_read_tokens=531, cache_write_tokens=2458)` — the product's measurement reproduced (the bulk of the input in the cache, `input_tokens` the fresh part) and now on the record. **ENH-024:** a session opened with `resume=<unknown id>` on Codex → `failed=True, session_gone=True`, the text Codex's stderr sentence; on Claude Code → `failed=True, session_gone=True`, the text its `errors[]` sentence — both from the provider files alone.

---

### [ARCH_CHANGE] 2026-09-20 — G5: what the architecture's documents gain from this phase
Topics: runtime, thread, wire, adapters, modes, usage, failure, attributes
Affects-phases: phase-45-truth-both-ways
Affects-specs: architecture/runtime.md#the-thread, architecture/wire.md#the-thread-crossed, architecture/adapters.md#the-modes-adapter, architecture/adapters.md#the-agent-adapter, architecture/testing.md
Detail: Additive, for `/sync-docs` at completion. `runtime.md` (the thread): `TurnRecord.failure` and `SessionGone` (D139); `turn(attributes=)` / `resume(attributes=)` (D140); `Spent` and `Usage` cache tokens (D141); the reserved attribute names now include the conversation's own keys (BUG-061); a provider's failed `Turn` is a `Failed` observation (BUG-060). `wire.md` (the thread, crossed): `session_gone` in the error kinds with `thread_id` and `session_id`; `attributes` on `turn/start` and `thread/resume`; `failure` on every turn record; protocol 3. `adapters.md` (the modes adapter): the modes table's `ask` row says the web is asked about; (the agent adapter): `Dialect.session_gone_matches`, `cache_*_tokens_at`, `BehaviourArg.template`; the JSONL session reads stderr. `testing.md`: the vendor-name invariant among the invariants; the 3.12–3.14 matrix among the runners.
