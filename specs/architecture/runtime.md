...
| `conversation.py` | `Conversation` — the governed turn as a primitive (D87): one provider session on the served registry, `turn(text, when=, on_question=)` as a run of one step streamed as events, `last` what it came to (`Turned`), `tools()`, `set_mode`, `add_root`, `steer`, `interrupt`; no record — a product that keeps its own takes this alone. A question a turn parks on purpose (`Parked`, D88) stays on `last.pending` for whoever keeps the record. `plan_limits` — the host's met with the current mode's, read at every turn (D109); `unmapped_behaviour` — the behaviour fields the mode set that the provider could not take, read off the session at every open (ENH-020); `Changed.unmapped` |
