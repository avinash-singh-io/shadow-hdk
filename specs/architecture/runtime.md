...
| `bindings.py` | `Ports`, `RunOptions`, `RunContext`, the contextvar (D2); `executing(step)` — the scope within which `current_run().step` is set, the component's invoke only; `spawn_options` — a child inherits its parent's context attributes (thread, turn, mode) unless handed its own (D74, BUG-030), and its plan limits are the meet of its parent's and the caller's (D109); `RunOptions.plan_limits`; `registered()`, `judge()`, `emit()` — what admission reads and writes, runtime-side (Phase 36) |
...
| `step.py` | `StepExecutor.invoke` — the seven moves; a component answering `Asked` parks the run and is resumed with the answer and what it kept (D57). A step that parks a **second** time in one leg says how many answers its task already holds (`replays`), and the next leg drains that many before handing the component the newest — LangGraph replays a task's resume values by index (BUG-055). A plan the step deferred runs after it closes (`run_deferred`, D112) |
| `compile.py` | `compile_composition`; the structural-hash cache (D11) |
| `state.py` | `RunState` TypedDict and its reducers; `plan` — the composition as JSON, seeded in the initial state and written by every node, so a parked run's shape survives where its state does (D116) |
| `loop.py` | `run`, `resume` — Started … Ended, end reasons, child forwarding; `resume` handed a composition whose digest differs from the checkpoint's `plan` admits it as an **amendment** (D116) — `Composed` and `plan_admitted(amendment=True)`, or `plan_refused` with the parked run untouched; `parked_composition(checkpointer, run_id)` — the shape a run parked with, for a settle |
...
| `approvals.py` | the other handle a host keeps: a component requests approval — or the person's input — **live** while its step runs, because a step holding a provider's session cannot park (D58); answered `Approve`, `Deny`, `ApproveAndAddRule` (D61), `Parked` (D88) or `Amend(composition, answer)` — a parked plan continued on a different shape, carried to whoever holds it (D116) |
| `children.py` | what a run is holding — spawn · send · release, and the records that survive a park (D16, D37); `Narrowed`, the pattern's ceiling applied as a second gate where the child is spawned, whichever side of the wire (D51). **Admission** (D108): `spawn` calls `admit()` before `run()` — the kernel's structural and existence checks under the limits in force, then every leaf's declared effects dry-judged in the child's own context to *name* `PlanAdmitted.asks` and `.refusals` (never to pre-empt a step's own judgement — D108/D121 as amended); `plan_refused` on the record and `PlanNotAdmitted` to the caller, nothing spawned. `defer`/`run_deferred` — a plan admitted now and run after its planner's step (D112); `amend` — a held child woken on a different composition, admitted first (D116) |
...
| `conversation.py` | `Conversation` — the governed turn as a primitive (D87): one provider session on the served registry, `turn(text, when=, on_question=)` as a run of one step streamed as events, `last` what it came to (`Turned`), `tools()`, `set_mode`, `add_root`, `steer`, `interrupt`; no record — a product that keeps its own takes this alone. A question a turn parks on purpose (`Parked`, D88) stays on `last.pending` for whoever keeps the record. `plan_limits` — the host's met with the current mode's, read at every turn (D109); `unmapped_behaviour` — the behaviour fields the mode set that the provider could not take, read off the session at every open (ENH-020); `Changed.unmapped`; `turn(attributes=)` — this turn's words, merged over the conversation's for this turn's judgements and never written back (D140); the conversation's own context keys (`thread`, `turn`, `mode`) are reserved attribute names beside the step's (BUG-061); a provider's failed `Turn` is a `Failed` observation, so the turn is `failed` (BUG-060), and `Turned.failure` says why when the kit can (`session_gone`, D139); `Spent` counts what the cache did (D141) |
| `threads.py` | `Thread` — a `Conversation` with a record: the container every product has (D62): a provider opened once and held across turns, resumed on its own session id when reopened (D76), the registry served for its lifetime under the host's name, each turn its own run; `set_mode` (policy, then the environment follows, then the provider — D64, D76), `add_root`, `tools()` — what the agent is offered now with the mode's judgement (D73), each with a `source` the port declares when it has one to declare (the wire's `RemoteComponents` says `host`) and derived from provenance otherwise; `workspace`, `environment_mode`; version 3 persists `ExecutionRequirements`, `execution` retains the accepted selection, and resume rechecks instead of trusting a stale construction decision (D96–D98); `WorkspaceChanged` and `ModeChanged` announced between turns; the questions a turn is waiting on kept on the record and a turn the host died in `parked` or `cancelled` at resume, `settle` resuming the parked act from the checkpointer on the composition it parked with (D80, D116) — a question the run parks on again is kept under the same handle, the agent told its call is still waiting (BUG-055); `amend(handle, composition, answer)` = `settle` with `Amend`; `plan_limits`, `unmapped_behaviour` (Phase 36); one holder per thread — a hold taken, renewed and released on the store, `ThreadHeld` naming another's — and `turn(when=)`: enqueue · reject · interrupt (D81); `turn(attributes=)` and `resume(attributes=)` — the turn's words for its judgements, the record's replaced and saved (D140); `TurnRecord.failure` and `SessionGone(thread_id, session_id, provider)` raised once the stream has ended, so a host can `fork` rather than guess (D139) |
...
| `planning.py` | `compose` — a plan proposed as a component (D110): no effects, the composition as input, every step's result as output; the same path as the loop's meta-tool — `children.spawn`, where admission (D108) refuses structure and existence before compile and names what each step will ask or refuse |
...
**Plan admission (D107–D111, Phase 36).** Before any of the above, a *plan* — a `Composition`
the agent, a resident CLI or the host proposed; a single call is a plan of one step — is judged
whole where it is spawned (`Children.spawn`, whichever side of the wire): its shape against the
`PlanLimits` in force (depth, fan-out, steps — narrowing host → mode → parent by `meet`, the
lease still the floor at run time), every component's existence, then every leaf's declared
effects dry-judged in the child's own context. Structure and existence *refuse* the plan —
`plan_refused` with every `PlanMismatch` in stable order, `PlanNotAdmitted` to the caller,
nothing compiled, nothing spawned; effects are *named* on `plan_admitted.asks` and `.refusals`,
and each step is still refused, parked or asked live at its own invocation through the path above
(D108, D121 as amended in Epic 0009). A refused plan is an observation the planner reads and
re-proposes on; the runtime never trims a proposal (D111). The kernel's `admit()` is pure and
measures; the runtime judges. `composition_digest()` is what the checkpoint's `plan` is compared
against on resume: a different composition is an amendment, admitted the same way (D116).
