...
| 30 | A product owns what it owns | **active** · `phase-30-a-product-owns-what-it-owns` | 29 | the governed turn without the record (`Conversation`); a park on purpose; the agent streams; tokens and running time on the record; the contract suites shipped and a `Questions` port; routed governance and typed refusals; a parked run behind a port of ours and the record versioned; sessions that idle out and a stream that survives a drop |
