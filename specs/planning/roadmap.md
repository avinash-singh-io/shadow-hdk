...
(Rule 6). The suite is **406 tests**, mypy strict over **81 source files**, every package at
**0.6.0**.

| | phases | state |
|---|---|---|
| done | 0 – 8 | every task ticked; each left the gate green |
| under way | 9 — the wire | group 0 of 5 (D19, D20) |
...
as a meta-tool (phase 8), D19 the graph state holds JSON and D20 the `Spent` event (phase 9).
...
| 9 | The wire | **IN PROGRESS** · 1 / 5 groups · `phase-9-the-wire` | 6, 7 | R3 → J2 | `serve` (JSON-RPC 2.0 over HTTP/2 + SSE), `--stdio`; schemas published; **`v0.1.0`** |
