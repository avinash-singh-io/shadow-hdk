...
| 36 | Plan admission | planned · **Epic 0009** | 33 | `PlanLimits` (order-bearing) + `admit()` pure in the kernel; admission inside `spawn` — structural → existence → effects, the mismatch list complete; `plan_admitted` / `plan_refused` and their fold; planning as a registered component so a resident CLI can plan; limits on the mode as live data; run-after-planner as a pattern field; the amend handle; wire/TS parity (D107–D112, D116, D121) |
